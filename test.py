...
def get_price_akakce(product_dict):
    ...
    for counter, (product_main_name, values) in enumerate(product_dict.items()):
        values = [{'i̇şlemci': 'AMD Ryzen 5 5500 4.20 Ghz 6 Çekirdek 19MB AM4 7nm İşlemci(MPK) ', 'anakart': 'MSI B450M-A PRO MAX 3466mhz(OC) M.2 AM4 mATX Anakart ', 'ram': 'GoodRam IRDM PRO 16GB(2x8) Deep Black 3600mhz CL18 DDR4 Ram (K3600D4V64L18S/16GDC) ', 'ekran kartı': 'MSI GeForce RTX 3060 Ventus 2X 8G OC 8GB GDDR6 128 Bit Ekran Kartı ', 'ssd': 'Addlink 512GB S68 NVMe Okuma 2100MB-Yazma 1700MB M.2 SSD (AD512GBS68M2P) ', 'kasa': 'BitFenix Garen 600W Tempered Glass USB 3.0 ATX Mid Tower Kasa ', 'soğutucu': 'AMD Wraith Stealth AMD Uyumlu Hava Soğutucu '}, 'https://static.sinerji.gen.tr/Images/LG/4XG-tqk-ekran-goruntusu-2023-07-06-165919.jpg', 'https://www.sinerji.gen.tr/sinerji-saraxy-ryzen-5-5500-16gb-512gb-nvme-m2-ssd-rtx3060-oyun-bilgisayari-p-47597', '18192']
        ...
        for product_category, product_name in product_details_dict.items():
            ...
            while retries < 30:
                try:
                    result = requests.get(link, headers=header, params=params, verify=False)
